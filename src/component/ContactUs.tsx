...
const ContactUs = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault(); // <-- Crucial: Prevent the default form submission (and redirection)
    ...
    try {
      // Manually send data to FormSubmit.co
      const response = await fetch("https://formsubmit.co/ajax/buddhapvtiti@gmail.com", {
        method: "POST",
        headers: {
          'Content-Type': 'application/json', // Use application/json for AJAX submissions
          'Accept': 'application/json'
        },
        body: JSON.stringify({
          _subject: "New Inquiry from College Website",
          // No _next needed here, as we're handling success locally
          name: formData.name,
      ...
      // You can check the response status if needed
      if (response.ok) {
        ...
        // Handle server errors or FormSubmit.co issues
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
        ...
        <div className="bg-white rounded-xl shadow-lg p-8">
          ...
          <div className="space-y-6">
            {/* IMPORTANT: Remove action and method from form, and add onSubmit to call your function */}
            <form onSubmit={handleSubmit}>
              {/* No need for hidden inputs _subject and _next here, we send them in the fetch body */}
              <div>
